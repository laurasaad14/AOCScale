...
## unidim(aoc1.itemsOnly) ## is it unidimensional?  bit unsure how to interpret : higher they are the more unidimeionsional
### not terrible
### psych::reliability(aoc1.itemsOnly)

## single factor
## f1 <- fa(aoc1.itemsOnly, nfactors=1)
## summary(f1)
## fa.lookup(f1, dictionary=names(aoc1.itemsOnly))
...
## f2 <- fa(aoc1.itemsOnly, nfactors=2)
## p2 <- principal(aoc1.itemsOnly,nfactors=2)
## factor.congruence(f2,p2)  ## how similar are they? --> pretty similar [looking for ~1 diagonal]

## fa.lookup(f2, dictionary=names(aoc1.itemsOnly))
## fa.diagram(f2)

## summary(f2)
## ### NOT 2 factors.  none of the items in second dimension are > .35!

## f2 <- fa(aoc1.itemsOnly, nfactors=2, rotate="varimax")
## fa.lookup(f2, dictionary=names(aoc1.itemsOnly))
...
## #######  3 factor solution ########
## ###################################
f3 <- fa(aoc1.itemsOnly, nfactors=3)
p3 <- principal(aoc1.itemsOnly,nfactors=3)
...
alpha(aoc1.itemsOnly)  ##
omega(aoc1.itemsOnly, nfactors=3)  ##
